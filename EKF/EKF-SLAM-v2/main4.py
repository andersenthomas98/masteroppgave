...
# Load data
estimator_file = open('estimator_log_12.txt', 'r')
line_file = open('line_log_12.txt', 'r')
normal_file = open('normal_log_12.txt', 'r')
...
for i in range(len(list(line_data_))):
    ...
    if np.sqrt(np.power(Px-Qx, 2) + np.power(Py-Qy, 2)) > 0.1 and np.sqrt(np.power(Px-Qx, 2) + np.power(Py-Qy, 2)) < 0.5:
