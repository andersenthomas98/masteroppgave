...
class ConfigParser:
    ...
    PARAMS = ['required_libs', 'dropped_libs', 'compilers', 'target']
    ...
    def parse_libs(self):
        for name, params in self.config.items():
            ...
            for lib, lib_params in self.libs_raw.items():
                lib_renamed = lib.format(target_name=params['target'])
                if lib_params:
                    self.libs[name][lib_renamed] = lib_params.copy()
                    if 'target_dir' in lib_params:
                        self.libs[name][lib_renamed]['target_dir'] = lib_params['target_dir'].format(target_name=params['target'])
                    if 'src_path' in lib_params:
                        self.libs[name][lib_renamed]['src_path'] = lib_params['src_path'].format(target_name=params['target'])
                else:
                    self.libs[name][lib_renamed] = {}

    def validate_config(self):
        for config_name, target_params in self.config.items():
            target = target_params['target']
            for required_param in self.PARAMS:
                if required_param not in target_params:
                    self.logger.warning('Missing param "{}" for "{}"'.format(required_param, config_name))
                    self.config[config_name][required_param] = []
            for param in target_params:
                if param not in self.PARAMS:
                    self.logger.error('Unexpected param "{}" for "{}"'.format(param, config_name))
                    raise AttributeError('Unexpected param "{}" for "{}"'.format(param, config_name))

            for lib in target_params['required_libs']:
                if lib not in self.libs[config_name]:
                    self.logger.error('Unexpected required lib "{}" for target "{}"'.format(lib, config_name))
                    raise AttributeError('Unexpected required lib "{}" for "{}"'.format(lib, config_name))

            for lib in target_params['dropped_libs']:
                if lib not in self.libs[config_name]:
                    self.logger.error('Unexpected dropped lib "{}" for target "{}"'.format(lib, config_name))
                    raise AttributeError('Unexpected dropped lib "{}" for "{}"'.format(lib, config_name))

            for lib in self.libs[config_name]:
                if lib not in target_params['required_libs'] and lib not in target_params['dropped_libs']:
                    self.logger.error('Lib "{}" not handled for target "{}"'.format(lib, config_name))
                    raise AttributeError('Lib "{}" not handled for "{}"'.format(lib, config_name))
...
if __name__ == '__main__':
    ...
    for config_name, target_params in config.items():
        target = target_params['target']
        logging.debug('Processing configuration {}'.format(config_name))
        for compiler_name in target_params['compilers']:
            ...
            compiler = compiler_class(libs[config_name])
            logging.debug('Compiler {}'.format(compiler_name))
            logging.debug('Compiler class {}'.format(compiler_class))

            os.makedirs(os.path.join('..', 'lib', '{target}', '{compiler}').format(target=target, compiler=compiler_name.casefold()), exist_ok=True)

            for lib in target_params['required_libs']:
                from_path = compiler.get_from_path(config_name, lib)
