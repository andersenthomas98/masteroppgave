...
void mapping_task(void *arg) {
  ...
  while(1) {
      ...
      if (ulTaskNotifyTake(pdTRUE, (TickType_t) 0) == pdPASS) {
        ...
        for (int i=0; i<line_buffer.len; i++) {
          for (int j=0; j<line_buffer.buffer[i].points.len; j++) {
            for (int k=0; k<line_buffer.len; k++) {
              for (int l=0; l<line_buffer.buffer[k].points.len; l++) {
                if (&(line_buffer.buffer[i].points.buffer[j]) == &(line_buffer.buffer[k].points.buffer[l]) && i != k) {
                    NRF_LOG_INFO("match!");
                    mqttsn_cluster_msg_t msg;
                    msg.cluster_id = i;
                    msg.point = (coordinate_t) {.x = line_buffer.buffer[i].points.buffer[j].x, .y = line_buffer.buffer[i].points.buffer[j].y};
                    publish_cluster_point("v2/robot/NRF_5/debug", msg, sizeof(msg), 0, 0);
                    TickType_t lastWakeTime = xTaskGetTickCount();
                    vTaskDelayUntil(&lastWakeTime, configTICK_RATE_HZ*0.1);

                    msg.cluster_id = k;
                    msg.point = (coordinate_t) {.x = line_buffer.buffer[k].points.buffer[l].x, .y = line_buffer.buffer[k].points.buffer[l].y};
                    publish_cluster_point("v2/robot/NRF_5/debug", msg, sizeof(msg), 0, 0);
                    lastWakeTime = xTaskGetTickCount();
                    vTaskDelayUntil(&lastWakeTime, configTICK_RATE_HZ*0.1);
                }
